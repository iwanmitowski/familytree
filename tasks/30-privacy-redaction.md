...
## Requirements
1. `internal/privacy` `PersonRedactionService` — single entry point `Redact(person PersonView, viewer ViewerContext) RedactedPerson` where `viewer.view ∈ {admin, public}` (a `family` tier is defined in the enum for the future but unused — document):
   - **admin**: everything;

...
## Requirements
1. `services/api/Dockerfile`: multi-stage; build with the Go version from go.mod, `CGO_ENABLED=0`, target `linux/arm64` via buildx platform; final image `gcr.io/distroless/static-debian12:nonroot`; healthcheck-compatible entrypoint (the `api healthcheck` subcommand from Task 04 — no curl in distroless).
2. `infra/oracle/docker-compose.prod.yml` (idea.md §19, all requirements):
   ...
   - api: `depends_on: postgres: condition: service_healthy`, healthcheck via `["CMD","/api","healthcheck"]`, `read_only: true` + `tmpfs: /tmp`, `restart: unless-stopped`, runs as nonroot;
   - one-shot `migrate` service (same api image, command `["/api","migrate","up"]`, profile `ops`) so migrations run safely before a new API rollout (idea.md §19);

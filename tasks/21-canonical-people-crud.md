...
## Requirements
1. `src/people` endpoints (admin role, audit entries on every mutation):
...
2. Places helper (`src/people` or `src/genealogy`): `upsertPlaceByText(trx, rawText)` → normalize (Task 19 `normalize`), find-or-create `places` row (`place_type='settlement'` default; hierarchy is admin-curated later). Never duplicate by normalized name.

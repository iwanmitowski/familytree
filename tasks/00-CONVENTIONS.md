...
- **Web:** Next.js (App Router) + TypeScript strict, Tailwind CSS, shadcn/ui, React Hook Form + Zod, TanStack Query, `@xyflow/react` (React Flow), `elkjs`, Auth.js, Vitest + Testing Library, Playwright. Node 22 LTS, npm (committed lockfile).
- **API:** Go (latest stable, e.g. 1.23+), chi router, pgx/v5 (pool), sqlc (generated code **is committed** under `services/api/db/generated`), goose SQL migrations (embedded in the binary via `embed.FS`, applied with an `api migrate up` subcommand), `log/slog` JSON logging.
- **DB:** PostgreSQL 16.
- **Infra:** Docker Compose, Caddy, GitHub Actions, Oracle Ampere A1 (`linux/arm64`), OCI Object Storage, `age` encryption, rclone.
- Module path for Go: `github.com/mitovski/familytree/services/api` (placeholder; adjust once a real remote exists and note it in PROGRESS.md).
...
- Request correlation: BFF forwards/generates `X-Request-Id`; Go logs it and echoes it in error bodies.
...
- Go: table-driven unit tests next to code; integration tests against real PostgreSQL guarded by build tag `integration` and env `DATABASE_URL` (dev compose), run with `go test -tags=integration ./...`.
- Web: Vitest + Testing Library; E2E via Playwright (Task 37).
- Standard verification commands (referenced by tasks as "standard Go/web verification"):
  - Go: `cd services/api && go vet ./... && go build ./... && go test ./...` (+ integration tag when the task touches the DB; start DB with `docker compose -f docker-compose.dev.yml up -d`).
  - Web: `cd apps/web && npm run lint && npm run typecheck && npm test -- --run && npm run build`.

## 8. Environment notes

- Development machine is Windows 11 with Docker Desktop. Run shell scripts via Git Bash or WSL. All `scripts/*.sh` target the Linux server/CI (bash + `set -euo pipefail`); keep local dev workflows runnable with plain `go`/`npm`/`docker compose` commands so Windows is not blocked.
...
- Schema changes → new goose migration (never edit an applied migration) + update `docs/data-model.md` (tables + ER diagram).

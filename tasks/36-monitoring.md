...
## Requirements
1. Metrics via `prom-client`, endpoint `GET /metrics` bound so it is reachable **only** on the internal Docker network (not routed by Caddy, not HMAC-exempt from outside — verify Caddyfile has no route to it):
   - HTTP: request counter + duration histogram labelled `{route, method, status}` (Hono route pattern, not raw path — bounded cardinality);
   - DB: `pg` Pool stats gauges (total/idle/waiting), query duration histogram via a pool query wrapper;
...
2. Turnstile rejections happen in the BFF: add `POST /v1/internal/abuse-events` `{kind: turnstile_rejected|honeypot|too_fast|rate_limited_bff}` (service auth, no payload details) — the BFF fires it on those events; the API counts + audit-logs them (idea.md §6 „audit записи за подозрителни заявки").
...
- Standard API verification; `shellcheck scripts/check-disk.sh`.

...
## Goal
Kysely migrations + basic typed queries for the staging/side tables: invites, submissions, submission people/relationships, consents, audit log, nonce store, idempotency keys.

## Requirements
1. New Kysely migration file(s) in `services/api/db/migrations` (sequential, each exporting `up(db)`/`down(db)` with raw DDL via the `sql` template, reversible). All PKs `UUID DEFAULT gen_random_uuid()`, all timestamps `TIMESTAMPTZ`, `created_at DEFAULT now()`. `updated_at` is app-managed (no triggers) — note this in `docs/data-model.md`.
...
4. Typed query helpers (Kysely, minimal set for later tasks), grouped per table under `src/<domain>/repo.ts`: insert/get/list-by-status for submissions; insert submission_person / submission_relationship; insert consent; insert audit entry; invite insert/get-by-token-hash/list/increment-used (guarded)/revoke; nonce insert + delete-expired; idempotency get/insert + delete-expired. Run `npm run codegen -w @familytree/api` to refresh `src/db/generated` and commit it.
...
## Verification
- Integration tests (`vitest.integration.config.ts`) covering the up/down/up cycle and the constraint cases above.

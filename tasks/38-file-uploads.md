...
## Verification
- API unit tests (sniffing matrix, EXIF-strip assertion) + integration (upload→fetch→delete against MinIO or a local dir-backed S3 stub); web tests for the upload UI states; extend the backup local run.
- Standard API + web verification.

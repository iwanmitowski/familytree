...
export default function SubmissionDetailPage() {
  ...
  return (
    <div className="grid gap-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {data.status === 'pending' && (
            <Button
              size="sm"
              onClick={() => transition.mutate({ path: 'start-review', successMsg: 'Прегледът започна' })}
            >
              Започни преглед
            </Button>
          )}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Хора ({data.people.length})</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Връзки ({data.relationships.length})</CardTitle>
        </CardHeader>
        <CardContent className="text-sm">
          {data.relationships.map((r, i) => (
            <div key={i} className="text-muted-foreground">
              {r.from_local_key} → {r.to_local_key} ({r.relationship_type})
            </div>
          ))}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Съгласия</CardTitle>
        </CardHeader>
        <CardContent className="text-sm">
